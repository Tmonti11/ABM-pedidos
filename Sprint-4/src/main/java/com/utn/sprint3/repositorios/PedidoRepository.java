...
public interface PedidoRepository extends BaseRepository<Pedido, Long> {
    ...
    @Query(value = "SELECT p FROM Pedido p WHERE  p.estadoPedido = :estado")
    List<Pedido> search(@Param("estado")EstadoPedido estado);

    @Query(value = "Select p FROM Pedido p WHERE Pedido.estadoPedido = :estadoPedido")
    List<Pedido> updateEstadoPedido(@Param("estadoPedido")  EstadoPedido estadoPedido);

    @Query(value = "Select p FROM Pedido p WHERE Pedido.estadoPago = :estadoPago")
    List<Pedido> updateEstadoPago(@Param("estadoPedido") EstadoPago estadoPago);
}
